...
class RestaurantAuthController extends Controller
{
    ...
    public  function login(Request $request)
    {
        ...
        if ($restaurant) {
            if (Hash::check($request->password, $restaurant->password)) {
                return $this->returnData(200, 'restaurants', new RestaurantResource($restaurant));
            }
        }
    ...
    public function register(Request $request)
    {
        ...
        $request->file('image')->storeAs('/profile', $image_name, 'restaurants');
    ...
    public function profile(Request $request)
    {
        try {
            $data = $request->user();
            return $this->returnData(200, 'restaurants', new RestaurantResource($data));
        } catch (\Exception $ex) {
    ...
    public function updateProfile(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $request->file('image')->storeAs('/profile', $image_name, 'restaurants');
            $data['image'] = $image_name;
            Storage::disk('restaurants')->delete('profile/' . $restaurant->image);
        } else {
